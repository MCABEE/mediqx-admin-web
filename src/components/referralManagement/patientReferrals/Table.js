...
//   const goToNextPage = () => {
//     if (page < totalPageCount) setPage(page + 1);
//   };
...
//   useEffect(() => {
//     fetchAgentPatientReferrals(page, limit, filter);
//   }, [page, filter, fetchAgentPatientReferrals]);

//   // Group referrals by createdAt date (for the date header rows)
//   const groupedByDate = referralsPatient.reduce((acc, referral) => {
//     const date = new Date(referral.createdAt).toDateString();
...
//             </tr>
//           )}
//           {!loading && referralsPatient.length === 0 && (
//             <tr>
//               <td colSpan={5} className="text-center p-4">
...
//           {/* ✅ Grouped by Date */}
//           {!loading &&
//             Object.entries(groupedByDate).map(([date, referrals]) => (
...
//                   const url = `/controlpanel/referral-management/patient-referrals-details/${referral.id}`;
//                   return (
//                     <Link
...
const PatientReferralTable = () => {
  ...
  useEffect(() => {
    fetchAgentPatientReferrals(page, limit, status, referredBy, serviceType, search);
  ...
  return (
    <div>
      ...
      <div className="w-full flex justify-between items-center bg-white border border-[#8888888c] rounded-[15px] mt-2 pt-4 pb-4 px-6 text-black font-semibold text-[16px]">
        <p className="font-semibold text-[32px]">{referralsPatient?.length || 0}</p>
        <button
