...
//               : "hover:text-gray-700"
//           }`}
//           title={uploading ? "Please wait until upload completes" : "Close"}
//         >
//           <AiOutlineClose size={20} />
//         </button>
...
function UploadExcelPopup({ onClose }) {
  ...
  const { generateUploadUrlDetail, confirmFileUpload, processExcelFile } =
    useNurseRegistrationStore();
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Confirm upload → returns { fileId }
      const { fileId: confirmedFileId } = await confirmFileUpload(
        fileId,
        "EXCEL_UPLOAD"
      );
      console.log("✅ Upload confirmed. Confirmed fileId:", confirmedFileId);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      ...
      {/* Loader Overlay */}
      {uploading && (
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-black/50 z-50 text-white">
          <AiOutlineLoading3Quarters
            size={50}
            className="animate-spin mb-4 text-white"
          />
