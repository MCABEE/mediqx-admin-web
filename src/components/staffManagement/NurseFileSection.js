...
const NurseFileSection = ({
  ...
  const handleFileSelect = (type, isQualificationFile) => async (e) => {
    ...
    if (type === "EXPERIENCE_CERTIFICATE" && !educationQualificationId) {
      setError(
        "Please save education qualification before uploading experience certificate."
      );
      return;
    }
  ...
  return (
    <>
      {fileSections.map(({ label, type, isQualificationFile }) => {
        ...
        const isDisabled =
          type === "EXPERIENCE_CERTIFICATE" && !educationQualificationId;
          ...
          >
            ...
            {file ? (
              <div className="flex items-center gap-4 ps-[52px]">
                ...
                <button
                  onClick={() => handleDelete(file.id)} // ✅ FIXED HERE
                  className="text-red-500 cursor-pointer"
                >
                  Delete
                </button>
