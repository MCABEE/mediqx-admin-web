    ...
    <div className="pb-4">
      ...
      {activeTab === "profile" && (
        ...
        <div className="flex gap-16 p-8 bg-white rounded-[15px] border border-[#BBBBBB] mt-2">
          ...
          <div className="flex flex-col gap-[10px] text-[16px] text-black">
            ...
            <span>{agentDetails?.district || "-"}</span>
            <span>{agentDetails?.city || "-"}</span>
            <span>{agentDetails?.lineFirst || "-"} {agentDetails?.lineSecond || ""}</span>
            <span>{agentDetails?.email || "-"}</span>
            <span>{agentDetails?.mobileNumber || "-"}</span>
            <span>{agentDetails?.referralType || "-"}</span>
          </div>
      ...
      {activeTab === "staffReferrals" && (
        <div className="space-y-4 py-2 mt-2">
          {loading && <p className="px-4 py-2">Loading referrals...</p>}
          {error && <p className="px-4 py-2 text-red-600">{error}</p>}
          {!loading && !error && referrals?.length === 0 && <p className="px-4 py-2">No referrals found.</p>}

          {referrals?.map((ref, idx) => (
            <div key={ref.id} className="mb-6 bg-white border border-[#BBBBBB] rounded-[15px]">
              <div className="flex border-b border-[#BBBBBB] py-2 w-full px-4 gap-40">
                <div className="bg-[#C0D8F6] rounded-[4px] p-2 font-semibold text-black">{(page - 1) * 10 + idx + 1}</div>
                <p className="text-[#666666] flex items-center">{new Date(ref.createdAt).toLocaleString() || ""}</p>
              </div>
              ...
              <div className="flex p-6 gap-16">
                ...
                <div className="flex flex-col gap-[10px] text-[16px] text-black">
                  ...
                  <span>{ref.dob ? new Date(ref.dob).toLocaleDateString("en-GB") : "-"}</span>
                  ...
                  <span>{ref.languagesdetails?.map(lang => lang.language).join(", ") || "-"}</span>
                  <span>{ref.workLocation || "-"}</span>
                  <span className={`text-[16px] ${ref.referralStatus?.toUpperCase() === "CONFIRMED" ? "text-green-600" : ref.referralStatus?.toUpperCase() === "CANCELLED" ? "text-red-600" : "text-blue-600"}`}>{ref.referralStatus || "-"}</span>
                </div>
              </div>
          ...
          {/* Pagination */}
          <div className="flex items-center justify-between gap-4 mt-4">
            <button onClick={() => setPage((p) => Math.max(p - 1, 1))} disabled={page === 1} className="px-4 py-2 rounded bg-gray-300 disabled:opacity-50">Previous</button>
            <span className="flex items-center px-2 font-semibold">Page {page} of {totalPages || 1}</span>
            <button onClick={() => setPage((p) => totalPages ? Math.min(p + 1, totalPages) : p + 1)} disabled={page === totalPages} className="px-4 py-2 rounded bg-gray-300 disabled:opacity-50">Next</button>
          </div>
        </div>
      )}
      ...
      {activeTab === "patientReferrals" && (
        <div className="space-y-4 py-2 mt-2">
          {loading && <p className="px-4 py-2">Loading referrals...</p>}
          {error && <p className="px-4 py-2 text-red-600">{error}</p>}
          {!loading && !error && referralsPatient?.length === 0 && <p className="px-4 py-2">No referrals found.</p>}

          {referralsPatient?.map((ref, idx) => (
            <div key={ref.id || `ref-${idx}`} className="mb-6 bg-white border border-[#BBBBBB] rounded-[15px]">
              <div className="flex border-b border-[#BBBBBB] py-2 w-full px-4 gap-40">
                <div className="bg-[#C0D8F6] rounded-[4px] p-2 font-semibold text-black">{(page - 1) * 10 + idx + 1}</div>
                <p className="text-[#666666] flex items-center">{new Date(ref.createdAt).toLocaleString() || ""}</p>
              </div>
              ...
              <div className="flex p-6 gap-16">
                ...
                <div className="flex flex-col gap-[10px] text-[16px] text-black">
                  ...
                  <span>{ref.serviceStartDate ? new Date(ref.serviceStartDate).toLocaleDateString("en-GB") : "-"}</span>
                  <span className={`text-[16px] ${ref.referralStatus?.toUpperCase() === "CONFIRMED" ? "text-green-600" : ref.referralStatus?.toUpperCase() === "CANCELLED" ? "text-red-600" : "text-blue-600"}`}>{ref.referralStatus || "-"}</span>
                </div>
              </div>
