...
function Page() {
  ...
  const [duplicateNames, setDuplicateNames] = useState([]);
  const [isSubmitting, setIsSubmitting] = useState(false); // ✅ Track submission state separately
  ...
  return (
    <div>
      ...
      <button
        className="bg-[#196BA5] text-white rounded-[15px] py-2 px-10 mt-2 cursor-pointer"
        onClick={handleSave}
        disabled={isSubmitting} // ✅ disable only while actually saving
      >
