...
function ManageDistrictsPage() {
  ...
  const handleUpdate = async () => {
    ...
    setApiError("");
    try {
      await updateDistrictById(
        checkedIds[0],
        { name: editName, stateId: editStateId },
        selectedStateId
      );
      setIsEditOpen(false);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="w-full bg-white border border-[#8888888c] text-base font-semibold flex justify-between px-6 rounded-[15px] mt-2">
        <div className="w-full flex items-center justify-between pt-[23px] pb-[19px]">
          <div className="flex item-center gap-[50px]">
            <Link
              href={
                "/controlpanel/data-manager/general-data/districts/add-districts"
              }
      ...
      <div className="relative bg-white px-6 py-2 mt-2" ref={dropdownRef}>
        ...
        >
          {statesList.find((s) => s.id === selectedStateId)?.name ||
            "Select State"}
        </div>
        {dropdownOpen && (
          <div
      ...
      {isLoading ? (
        ...
        <>
          ...
          {isDeleteConfirmOpen && checkedIds.length === 1 && (
            <ConfirmDeletePopup
              districtName={
                listedDistricts.find((d) => d.id === checkedIds[0])?.name
              }
