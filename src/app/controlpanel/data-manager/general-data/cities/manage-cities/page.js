...
// function ConfirmDeletePopup({ id, cityName, onConfirm, onCancel }) {
//   return (
...
//   // Handle checkbox toggle (single selection)
...
//     setApiError("");
//     try {
//       await updateCityById(checkedIds[0], { name: editName });
//       setIsEditOpen(false);
//     } catch (error) {
...
//     if (checkedIds.length !== 1) return;
//     await deleteCityById(checkedIds[0]);
...
function ManageCitiesPage() {
  ...
  setApiError("");
  try {
    await updateCityById(checkedIds[0], { name: editName }, selectedDistrictId, page);
    setIsEditOpen(false);
  } catch (error) {
    setApiError(error.message || "Failed to update city.");
  }
