...
// function ConfirmDeletePopup({ id, location, onConfirm, onCancel }) {
//   return (
...
//           <strong>{location.state}, {location.district}, {location.city}</strong>?
//         </p>
...
//   const [editData, setEditData] = useState({ state: "", district: "", city: "", pincode: "" });
...
//           <Link href="/controlpanel/data-manager/general-data/location/add-location" className="text-black">
//             Add
//           </Link>
...
//             <div key={loc.id} className="bg-white flex items-center gap-2 px-6 py-2 mt-2 flex-wrap">
//               <div className="border border-[#8888888c] py-2 px-4 rounded-[15px]">
//                 {(idx + 1 + (page - 1) * 10).toString().padStart(2, "0")}
//               </div>

//               <input type="text" value={loc.state} readOnly className="w-[150px] border border-[#8888888c] py-2 px-4 rounded-[15px] outline-none" />
//               <input type="text" value={loc.district} readOnly className="w-[150px] border border-[#8888888c] py-2 px-4 rounded-[15px] outline-none" />
//               <input type="text" value={loc.city} readOnly className="w-[150px] border border-[#8888888c] py-2 px-4 rounded-[15px] outline-none" />
//               <input type="text" value={loc.pincode} readOnly className="w-[100px] border border-[#8888888c] py-2 px-4 rounded-[15px] outline-none" />

//               <input type="checkbox" className="size-6 rounded-[15px]" checked={checkedIds.includes(loc.id)} onChange={() => handleCheckbox(loc.id)} />
//             </div>
//           ))}

//           <div className="flex justify-center items-center gap-2 mt-4">
//             <button disabled={page <= 1} onClick={() => setPage(page - 1)} className="px-3 py-1 bg-gray-200 rounded disabled:opacity-50">Previous</button>
...
//             <button className="bg-[#196BA5] text-white rounded-[15px] py-2 px-10 cursor-pointer" disabled={checkedIds.length !== 1} onClick={() => setEditPopupOpen(true)}>Edit</button>
//             <button className="bg-[#d9534f] text-white rounded-[15px] py-2 px-10 cursor-pointer" disabled={checkedIds.length !== 1} onClick={() => setConfirmPopupOpen(true)}>Remove</button>
//           </div>
...
//                 <input className="w-full border p-2 mb-2 rounded" placeholder="State" value={editData.state} onChange={(e) => setEditData({ ...editData, state: e.target.value })} />
//                 <input className="w-full border p-2 mb-2 rounded" placeholder="District" value={editData.district} onChange={(e) => setEditData({ ...editData, district: e.target.value })} />
//                 <input className="w-full border p-2 mb-2 rounded" placeholder="City" value={editData.city} onChange={(e) => setEditData({ ...editData, city: e.target.value })} />
//                 <input className="w-full border p-2 mb-2 rounded" placeholder="Pincode" maxLength={6} value={editData.pincode} onChange={(e) => {
//                   const val = e.target.value.replace(/\D/g, "");
//                   setEditData({ ...editData, pincode: val });
//                 }} />
//                 <div className="flex justify-end gap-2 mt-4">
//                   <button onClick={() => setEditPopupOpen(false)} className="px-4 py-2 border rounded hover:bg-gray-100">Cancel</button>
...
//           {confirmPopupOpen && checkedIds.length === 1 && (
//             <ConfirmDeletePopup
//               id={checkedIds[0]}
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="flex gap-3 my-6 px-6">
            <button className="bg-[#196BA5] text-white rounded-[15px] py-2 px-10 cursor-pointer" disabled={checkedIds.length !== 1} onClick={() => setEditPopupOpen(true)}>Edit</button>
            <button className="bg-[#196BA5] text-white rounded-[15px] py-2 px-10 cursor-pointer" disabled={checkedIds.length !== 1} onClick={() => setConfirmPopupOpen(true)}>Remove</button>
          </div>
