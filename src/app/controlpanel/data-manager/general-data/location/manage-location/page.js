...
function ManageLocations() {
 ...
 const handleUpdate = async () => {
  ...
  if (!editData.state || !editData.district || !editData.city || !/^\d{6}$/.test(editData.pincode)) {
      ...
      {["state", "district", "city", "pincode"].map((field) => (
        <input
          ...
          className={`text-base px-4 rounded-[15px] w-full h-[40px] border ${
            useLocationStore.getState().duplicateIndexes.includes(0) ? "border-red-500" : "border-[#BBBBBB]"
          } bg-white outline-none`}
          placeholder={field.charAt(0).toUpperCase() + field.slice(1)}
          value={editData[field]}
          maxLength={field === "pincode" ? 6 : undefined}
          onChange={(e) => {
            const val = field === "pincode" ? e.target.value.replace(/\D/g, "") : e.target.value;
