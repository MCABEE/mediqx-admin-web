...
//       // Fetch list by category with pagination
//    fetchItems: async (category, page = 1, limit = 10) => {
//   set({ isLoading: true, error: null, success: false });
//   try {
//     const response = await fetchList(category, page, limit); // Your API call
//     // response.data holds the paginated data object
//     const data = response.data;

//     set((state) => ({
//       listedItems: {
//         ...state.listedItems,
//         [category]: data?.[category] || [], // e.g. data.specializations
//       },
//       pagination: {
//         ...state.pagination,
//         [category]: {
...
//       // Update item by category and ID
//       updateItemById: async (category, id, data) => {
...
//           await get().fetchItems(category, page, limit);
//           set({ success: true });
//         } catch (error) {
...
//       // Delete item by category and ID
//       deleteItemById: async (category, id) => {
//         if (!CATEGORIES.includes(category)) {
//           set({ error: `Unknown category: ${category}` });
//           return;
//         }
...
//           set({ error: null });
//           await get().fetchItems(category, newPage, limit);
...
//   persist(
//     (set, get) => ({
//       // Data store per category, initialized as empty arrays
//       listedItems: {
//         specializations: [],
//         qualifications: [],
//         "working-departments": [],
...
//         specializations: { page: 1, limit: 10, totalPages: 0, totalItems: 0 },
//         qualifications: { page: 1, limit: 10, totalPages: 0, totalItems: 0 },
//         "working-departments": { page: 1, limit: 10, totalPages: 0, totalItems: 0 },
//         skills: { page: 1, limit: 10, totalPages: 0, totalItems: 0 },
//       },
...
//       // Fetch list by category with pagination, adapting to API response structure
//       fetchItems: async (category, page = 1, limit = 10) => {
//         if (!CATEGORIES.includes(category)) {
//           set({ error: `Unknown category: ${category}` });
//           return;
...
//         set({ isLoading: true, error: null, success: false });
//         try {
//           const response = await fetchList(category, page, limit);
//           const data = response.data;

//           // Extract the items array from API response using mapping
//           const key = STORE_TO_API_FIELD_MAP[category];
...
const useManageProfessionalsStore = create(
  persist(
    (set, get) => ({
      ...
      // fetchItems: async (category, page = 1, limit = 10, professionalCategory = "REG_NURSES") => {
      //   if (!CATEGORIES.includes(category)) {
      //     set({ error: `Unknown category: ${category}` });
      //     return;
      //   }
      //   set({ isLoading: true, error: null, success: false });
      //   try {
      //     // API call with category as query param
      //     const response = await fetchList(category, page, limit, professionalCategory);
      //     const data = response.data;
      //     const key = STORE_TO_API_KEY_MAP[category];
      //     set((state) => ({
      //       listedItems: {
      //         ...state.listedItems,
      //         [category]: data?.[key] || [],
      ...
      fetchItems: async (category, page = 1, limit = 10, professionalCategory ) => {
  if (!CATEGORIES.includes(category)) {
    set({ error: `Unknown category: ${category}` });
    return;
  }
