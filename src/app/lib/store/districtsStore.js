...
// import { addDistricts, getStates } from "@/api/districtsApi";

// const useDistrictStore = create(
//   persist(
//     (set, get) => ({
//       // District inputs with name and selected stateId
...
//       // For paginated states used in edit popup
//       statesList: [],
//       statesPage: 1,
//       statesTotalPages: 0,
...
//       addDistrictInput: () =>
//         set((state) => ({ districts: [...state.districts, { name: "", stateId: "" }] })),
...
//         try {
//           // Only keep districts where name and stateId are filled
//           const filtered = get().districts.filter(
//             (d) => d.name.trim() !== "" && d.stateId.trim() !== ""
//           );
//           if (filtered.length === 0) {
//             set({ error: "Please enter at least one valid district with state selected." });
//             return;
//           }
//           await addDistricts(filtered);
//           set({ success: true, districts: [{ name: "", stateId: "" }], error: null });
//         } catch (error) {
...
//       // Paginated states for select dropdown
//       statesList: [],
//       page: 1,
//       totalPages: 0,
//       isStatesLoading: false,
...
//         isStatesLoading: state.isStatesLoading,
//         error: state.error,
//         success: state.success,
//          listedDistricts: state.listedDistricts,
//         page: state.page,
       
//         statesPage: state.statesPage,
//         statesTotalPages: state.statesTotalPages,
...
//           set({ success: true, districts: [{ name: "", stateId: "" }], error: null });
//         } catch (error) {
//           set({ error: error.message || "Failed to save districts.", success: false });
//         } finally {
//           set({ isLoading: false });
//         }
//       },
...
//         set({ isLoading: true, error: null });
//         try {
//           // Pass stateId param if provided
//           const response = await getDistricts(page, get().limit, stateId);
//           set({
//             listedDistricts: response.data.districts || [],
//             page: response.data.page || page,
//             limit: response.data.limit || get().limit,
